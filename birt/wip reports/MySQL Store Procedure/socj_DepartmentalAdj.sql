...
DROP PROCEDURE IF EXISTS `socj_DepartmentalAdj` $$
CREATE DEFINER=`bvu`@`10.100.4.13` PROCEDURE `socj_DepartmentalAdj`(IN fromDate VARCHAR(50), IN toDate VARCHAR(50),IN company VARCHAR(50))
BEGIN
select Department,
d.department_name,
d.rollup_division,
...
order by Department,Direction DESC,InFromOutTO, job DESC) as a
JOIN dept_rollup d
