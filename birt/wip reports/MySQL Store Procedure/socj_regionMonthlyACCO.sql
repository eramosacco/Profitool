...
DROP PROCEDURE IF EXISTS `socj_RegionMonthlyACCO` $$
CREATE DEFINER=`bvu`@`10.100.4.13` PROCEDURE `socj_RegionMonthlyACCO`(IN YearEnd VARCHAR(50))
BEGIN
  ...
  OPEN cur1;
  #OPEN cur2;
  DELETE from socj_regionMonthlyACCO;
  read_loop: LOOP
    FETCH cur1 INTO a;
    IF done THEN
      LEAVE read_loop;
    ...
    SET @s = Concat('Insert into socj_regionMonthlyACCO (Department, DepartmentName,DepartmentDivision,DepartmentGroup,',col,')
select p.pm_dept as Department , UPPER(r.department_name) as DepartmentName,
...
from soc_journal j
LEFT join (select * from soc_journal where socj_date_effective = "',prevMonth,'" and socj_coid = "ACCO" and socj_currentx = "X") s
ON j.socj_job = s.socj_job and j.socj_phase = s.socj_phase

JOIN socj_pm p ON p.job = j.socj_job and p.phase = j.socj_phase
...
where j.socj_date_effective = "',a,'" and j.socj_coid = "ACCO" and j.socj_currentx = "X"
...
ON DUPLICATE KEY UPDATE ',col,' = VALUES(',col,')');
  ...
  CLOSE cur1;
  SELECT * from socj_regionMonthlyACCO;
END $$
