...
DROP PROCEDURE IF EXISTS `socj_comparative` $$
CREATE DEFINER=`bvu`@`10.100.4.13` PROCEDURE `socj_comparative`(IN currYearEnd VARCHAR(50), IN currMonthEnd VARCHAR(50), IN preYearEnd VARCHAR(50), IN preMonthEnd VARCHAR(50),IN company VARCHAR(50))
BEGIN
...
if(cast(sum(((IFNULL(j.socj_GP_curr_yr,0) + IFNULL(j.socj_GP_prior_yr,0) +IFNULL(j.socj_CostAct,0)) - ((IFNULL(s.socj_GP_curr_yr,0) + IFNULL(s.socj_GP_prior_yr,0) +IFNULL(s.socj_CostAct,0)))) * pm_pct/100) as decimal(2)) = cast(sum(j.socj_GP_curr_yr * pm_pct/100) as decimal(2)) , 0,
...
(sum(j.socj_GP_curr_yr * pm_pct/100))))*100) as 'am'
...
WHERE j.socj_Date_effective = currMonthEnd and j.socj_coid  = company COLLATE latin1_general_cs and p.report_date = currMonthEnd and
j.socj_currentx ="X"
group by pm_clk
order by pm_dept;
...
WHERE j.socj_Date_effective = preMonthEnd and j.socj_coid  = company COLLATE latin1_general_cs and p.report_date = preMonthEnd and
j.socj_currentx ="X"
group by pm_clk
order by pm_dept
...
select * from socj_comparatives order by pm_dept,pm_name;
END $$
