...
DROP PROCEDURE IF EXISTS `socj_DeptAdjDetailSimple3` $$
CREATE DEFINER=`bvu`@`10.100.4.13` PROCEDURE `socj_DeptAdjDetailSimple3`(IN fromDate VARCHAR(50), IN toDate VARCHAR(50),IN company VARCHAR(50))
BEGIN
select "1" as bigRule, (l.department+0) as AdjDept,"-->" as Direction,pm_dept as InFromOutTo, (j.socj_job +0) as job, (j.socj_phase+0) as phase, UPPER(l.name) as JobName,
...
select  "1" as bigRule, pm_dept as AdjDept, "<--" as Direction , (l.department+0) as InFromOutTo,(j.socj_job +0) as job, (j.socj_phase+0) as phase, UPPER(l.name) as JobName,
(l.department+0) as CostDept, pm_clk, pm_name, pm_pct, pm_dept as pm_dept,
(IFNULL(j.socj_RevEst_Total,0) - IFNULL(s.socj_RevEst_Total,0)) * pm_pct/100 as Bookings,
...
and IF(l.department = 62 && p.pm_dept = 68,1,0)=0 and IF(l.department = 68 && p.pm_dept = 62,1,0)=0
UNION
select "2" as bigRule, (l.department+0) as AdjDept,"-->" as Direction,pm_dept as InFromOutTo, (j.socj_job +0) as job, (j.socj_phase+0) as phase, UPPER(l.name) as JobName,
(l.department+0) as CostDept, pm_clk, pm_name, pm_pct, pm_dept as pm_dept,
(IFNULL(j.socj_RevEst_Total,0) - IFNULL(s.socj_RevEst_Total,0)) * pm_pct/100 as Bookings,
...
UNION
select  "2" as bigRule, pm_dept as AdjDept, "<--" as Direction , (l.department+0) as InFromOutTo,(j.socj_job +0) as job, (j.socj_phase+0) as phase, UPPER(l.name) as JobName,
(l.department+0) as CostDept, pm_clk, pm_name, pm_pct, pm_dept as pm_dept,
(IFNULL(j.socj_RevEst_Total,0) - IFNULL(s.socj_RevEst_Total,0)) * pm_pct/100 as Bookings,
...
order by bigRule, AdjDept,Direction DESC,InFromOutTO, job DESC;
END $$
