...
DROP PROCEDURE IF EXISTS `socj_Product` $$
CREATE DEFINER=`bvu`@`10.100.4.13` PROCEDURE `socj_Product`(IN fromDate VARCHAR(50), IN toDate VARCHAR(50),IN company VARCHAR(50))
BEGIN
  select UPPER(e.product_description) as ProductDescription,UPPER(e.branch_name) as BranchName,
#Booking
...
group by UPPER(e.product_description) , UPPER(e.branch_name)#, j.socj_coid# v, socj_job
order by e.product_description ,e.branch_name;
